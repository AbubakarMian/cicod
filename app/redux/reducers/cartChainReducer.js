...
console.log("rr#@e",total_price_with_tax,total_price)
            ...
            case ADD_DELIVERY_FEE_TO_COST:
               ...
               // initialState.cart_detail.total_price_with_tax+=parseFloat(action.value)
                return {...state,cart_detail:{...state.cart_detail,delivery_fee:action.value,total_price_with_tax:total}}
            break;
            case REMOVE_DELIVERY_FEE_TO_COST:
                ...
                // updateCart();
               // initialState.cart_detail.total_price_with_tax+=parseFloat(action.value)
               return {...state,cart_detail:{...state.cart_detail,total_price_with_tax:state.cart_detail.total_price_with_tax-state.cart_detail.delivery_fee,delivery_fee:0}}
        ...
        case REMOVE_FROM_CART_CHAIN:
            ...
            for(let i=0;i<cart.length;i++){
                console.log("prodqy$",cart[i].purchased_quantity)
                total_price_r = total_price_r + (cart[i].purchased_quantity * cart[i].price);
                vat_amount_r = vat_amount_r + (cart[i].purchased_quantity *(cart[i].vat_amount==null?0:cart[i].vat_amount));
          ...
          //  console.log("rfew@3",cart)
            ...
            total_price_with_tax_r = (parseFloat(total_price_r)+parseFloat(tax_r)).toFixed(2)
        console.log("remov#$#@e",total_price_with_tax_r,total_price_r)
                    // updateCart();
                    return { ...state ,cart,cart_detail:{total_price:total_price_r,total_price_with_tax:total_price_with_tax_r,tax:tax_r,has_vat:has_vat_r,vat_percent:vat_percent_r,vat_amount:vat_amount_r}};
        ...
        case REMOVE_PRODUCT_FORM_CART_CHAIN:
            ...
            for(let i=0;i<cart.length;i++){
                console.log("prodqy$",cart[i].purchased_quantity)
                total_price_rp = total_price_rp + (cart[i].purchased_quantity * cart[i].price);
                vat_amount_rp = vat_amount_rp + (cart[i].purchased_quantity *(cart[i].vat_amount==null?0:cart[i].vat_amount));
          ...
          //  console.log("rfew@3",cart)
            ...
            total_price_with_tax_rp = (parseFloat(total_price_rp)+parseFloat(tax_rp)).toFixed(2)
        console.log("remov#$#@e",total_price_with_tax_rp,total_price_rp)
                    // updateCart();
                    return { ...state ,cart,cart_detail:{total_price:total_price_rp,total_price_with_tax:total_price_with_tax_rp,tax:tax_rp,has_vat:has_vat_rp,vat_percent:vat_percent_rp,vat_amount:vat_amount_rp}};
