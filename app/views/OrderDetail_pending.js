...
class OrderDetail_pending extends React.Component {
      ...
      .then(async responseJson => {
        ...
        if (responseJson.status === 'success') {
          ...
          for (let i = 0; product_items.length > i; i++) {
            total_ammount =
              total_ammount +
              parseFloat(product_items[i].price) *
                parseFloat(product_items[i].quantity);
          }
          ...
          this.setState({
            ...
            amount_paid_from_credit_limit:
              resdata.amount_paid_from_credit_limit,
            bodyOrder: {
  ...
  send_order_confirmation() {
    let url =
      Constants.orderslist + '/' + this.state.order_id + '?action=send_invoice';
    console.log();
    let postData = {
      method: 'GET',
      headers: {
                  ...
                  <FlatList
                    ...
                    renderItem={({item, index, separators}) => (
                      <View style={({}, styles.invoiceRow)}>
                        <View
                          style={{flexDirection: 'column', width: width - 50}}>
                          ...
                          <View
                            ...
                            <View style={{flex: 2, flexDirection: 'row'}}>
                              ...
                              <Text style={{fontSize: 13, color: '#929497'}}>
                                {item.price}{' '}
                              </Text>
                            </View>
                            <View style={{flex: 2, flexDirection: 'row'}}>
...
<View
              ...
              <View style={[{}, styles.noteView]}>
                <Text
                  style={[
                    {color: '#929497', marginLeft: 10, marginBottom: 10},
                    fontStyles.bold15,
                  ]}>
                  Notes
                </Text>
