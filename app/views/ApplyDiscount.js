...
class ApplyDiscount extends React.Component {
  constructor(props) {
    ...
    this.state = {
      value3Index:
        this.props.orderDiscountReducer.discount_type != 'percentage' ? 1 : 0,
      isChecked: false,
      discount_amount: '',
    };
  }
  ...
  setDiscount(discount_amount) {
    if (
      ...
      this.props.orderDiscountReducer.discount_amount == discount_amount
    ) {
      return;
    }
    console.log('discount_amount set discount', discount_amount);
    let discount_type = '';
    if (isNaN(discount_amount) && isNaN(parseFloat(discount_amount))) {
      //|| discount_amount==0
      discount_amount = '0';
    }
  ...
  render() {
    ...
    if (
      this.props.route.params.discount_amount != '' &&
                ...
                {radio_props_per.map((obj, i) => (
                  <RadioButton
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      backgroundColor:
                        this.state.value3Index === i ? '#F6EBEB' : '#fff',
                    }}>
                    <RadioButtonInput
                      obj={obj}
                      index={i}
