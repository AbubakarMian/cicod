...
class CreateQuickInvoice extends React.Component {
    ...
    sendQuickInvoice(send){
        ...
        let bodyData={
            ...
            discount_percent:this.state.discountType=="Percentage"?this.state.discount:null,
            discount_fixed:this.state.discountType!="Percentage"?this.state.discount:null,
        ...
        fetch(Constants.quickInvoice, postData)
            ...
            .then(async responseJson => {
                ...
                if (responseJson.status === "success") {
                    ...
                    if (send==1) {
            this.setState({error_title:"Invoice Sent Successfully",isShowError:true,isInvoiceSuccess:true})

                    // Alert.alert("Info ","Invoice Sent Successfully")
                        
                    } else {
            this.setState({error_title:"Invoice Saved Successfully",isShowError:true,isInvoiceSuccess:true})
...
alert(e.target.text)
 ...
 this.setState({discount_amount,products:array_products,isShowProductModal:false,product_name:"",quantity:0,price:0,modal_total:0,sub_total:sub_total,total_amount,isEdit:false})
        }else{
 this.setState({ tax_amount:0, discount_amount:0,products:array_products,isShowProductModal:false,product_name:"",quantity:0,price:0,modal_total:0,sub_total:0,total_amount:0,isEdit:false})
    ...
    pushProductToArray=()=>{
       ...
       array_products.push({name:product_name,quantity,unit_price:price,index:Date.now()})
//        let sub_total=0;
//        for (let index = 0; index < array_products.length; index++) {
//            const product = array_products[index];
//            console.log("#EEpr",product)
...
this.productStateCompute(array_products)
    ...
    handleChangeDiscount=(numb)=>{
        ...
        let discount=0;
        if(numb==""){
            total_amount=this.state.total_amount+parseInt(this.state.discount_amount);

            this.setState({discount:0,total_amount,discount_amount:0})
        }else{
        if (this.state.discountType=="Percentage") {
            discount=(parseFloat(numb)/100)*this.state.sub_total;
            total_amount=(this.state.sub_total + parseFloat(this.state.tax_amount))-discount
        } else {
            total_amount=(this.state.sub_total +this.state.tax_amount)-parseFloat(numb)
            discount=numb
            // total_amount=this.state.sub_total-discount
        }
    ...
    handleChangeTax=(tax_percent)=>{
        ...
        if (tax_percent=="") {
            ...
            console.log("uyu#@",this.state.discount_amount,"kk",this.state.tax_amount)
            // tax_amount=(parseInt(tax_loc)/100)*this.state.total_amount;
            total_amount=this.state.total_amount-parseFloat(this.state.tax_amount);
            this.setState({total_amount,tax_amount:0,tax_percent:0})
        }else{
            tax_loc=tax_percent;
            tax_amount=(parseFloat(tax_loc)/100)*this.state.sub_total;
            total_amount=this.state.sub_total+(parseFloat(tax_amount)+this.state.discount_amount);
...
this.productStateCompute(newArr)
 ...
 this.setState({index:item.index, isEdit:true,isShowProductModal:true,product_name:item.name,quantity:item.quantity,price:item.unit_price,modal_total})
        ...
        <AwesomeAlert
          ...
          onCancelPressed={() => {
              if (this.state.isInvoiceSuccess) {
                this.setState({isShowError:false},()=>this.props.navigation.replace("QuickInvoice"))
              } else {
                ...
                <ScrollView>
                    ...
                    <TouchableOpacity
                        ...
                        <View style={[{}, styles.backHeaderRowView]}>
                            ...
                            <View style={[{}, styles.backHeadingView]}>
                                <Text style={[{}, styles.backHeadingText]}>CREATE INVOICE</Text>
                            </View>
                        ...
                        <View style={[{}, styles.customerContainerView]}>
                            ...
                            <TouchableOpacity style={[{}, styles.customerContaineraddBtnView]}
                                onPress={() => this.props.navigation.navigate('AddCustomerInvoice')}
                            >
                                <Icon name="plus-circle" size={20} color={'#fff'} />
                                <Text style={[{}, styles.customerContaineraddBtnText]}>{this.props.customer.name == '' || this.props.customer.name == undefined ?"Add":"Change"}</Text>
                            </TouchableOpacity>
                            ...
                            {(this.props.customer.name == '' || this.props.customer.name == undefined) ?
                                <View style={[styles.customerContainerView,{flexDirection:"row"}]}>
                                    <Icon name="user-circle" size={20} color="#D8D8D8" />
                                    <Text style={[{marginLeft:10}, styles.customerContainerheading]}>No Customer added</Text>
                                ...
                                : <View style={[{}, styles.userDEtailCOntainer]}>
                                    ...
                                    <Text style={[ styles.usetDetailInfoText,{ marginLeft: 10,}]}>{this.props.customer.email}</Text>
                                        </View>
                                    </View>
                        ...
                        <View style={{flexDirection:"row",justifyContent:"space-between",alignItems:"center",marginTop:0,marginBottom:10,padding:10}}>
                            <Text style={{color:"#929497", fontWeight:"bold",fontSize:15}}>Select Due Date</Text>
                       

                            <View style={{ flex:1}}>
                            <TouchableOpacity
                                style={[{ flexDirection: 'row', maxWidth: width / 2, height: 40, borderRadius: 5, marginTop: 5, marginLeft: 51, backgroundColor: '#fff', alignItems: 'center' }]}
                    ...
                    />
                        ...
                        <View style={[ styles.OrderDetailContainer]}>
                            <View style={[ styles.OrderDetailHeadingRow,{ padding:10, borderBottomColor: '#E6E6E6',  borderBottomWidth: 0.25,paddingBottom:10}]}>
                                <Text style={[{}, styles.OrderDetailHeadingRowText]}>Product/ Service</Text>
                                {(this.state.products.length != 0) ?
                                    <Text style={[{}, styles.OrderDetailNotificationText]}>{this.state.products.length ?? 0}</Text>
                                    : null}
                            </View>
                            ...
                            {(this.state.products.length != 0) &&
                                ...
                                <FlatList
                                    ...
                                    renderItem={({ item, index, separators }) => (
                                        <View style={[{width:"100%", alignItems:"flex-start",flexDirection: 'column',flex:1 ,borderBottomColor: '#E6E6E6',  borderBottomWidth: 0.25,paddingBottom:10,marginTop:10}]}>
                                            <View style={[styles.OrderDetailDataCOntainerRow,{justifyContent:"space-between",alignItems:"flex-start",flexDirection:"row", width: width - 40}]}>
                                               <View style={{flexDirection:"row",justifyContent:"center",alignItems:"center"}}>
                                                 <TouchableOpacity onPress={()=>this.removeProductFromCart(item,index)}>
                                                 <Icon name="close" size={20} color="#B1272C" />
                                                 </TouchableOpacity>
                                               <Text style={{color:"#4E4D4D",fontSize:14,fontWeight:"bold",marginLeft:10}}>{item.name}</Text>
                                               </View>

                                                <Text style={{color:"#4E4D4D",fontSize:14,fontWeight:"bold"}}>{this.props.currency.currency}{item.unit_price*item.quantity}</Text>
                                            </View>
                                            <View style={[styles.OrderDetailDataCOntainerRow,{justifyContent:"space-between",alignItems:"flex-start",flexDirection:"row", width: width - 40,marginTop:10}]}>
                                                <Text style={{color:"#929497",fontSize:14}}>{item.quantity} x {this.props.currency.currency} {item.unit_price}</Text>

                                                <TouchableOpacity onPress={()=>this.editProduct(item)}  style={{padding:10}}>
                                                <Icon name="long-arrow-right" size={20} color="#929497" />
...
<View style={[{justifyContent:"center",alignItems:"center",marginTop:20}]}>
                        ...
                        >
                            <View style={[styles.customerContaineraddProductView,{alignSelf:"center",marginLeft:60}]}>
                        ...
                        {this.state.products.length>0 &&(
                            ...
                            <View style={[ styles.OrderDetailHeadingRow,{padding:10}]}>
                                <Text style={[{}, styles.OrderDetailHeadingRowText]}>Discount</Text>
                                
                            </View>
                            <View style={{ borderWidth: 0.25, borderColor: '#E6E6E6', width: width - 20, alignSelf: 'center', marginTop: 10 }}></View>

                            <View style={{flexDirection:"row",alignItems:"center"}}> 
                                <View style={{width:150,marginRight:10,marginTop:1, borderBottomColor:"#E6E6E6",borderBottomWidth:1,}}>
                                <Picker 
                                selectedValue={this.state.discountType}
                                     onValueChange={(itemValue,itemLabel, itemIndex) =>{
                                        ...
                                        if (itemValue=="Percentage") {
                                            discount=(parseInt(this.state.discount)/100)*this.state.sub_total;
                                            total_amount=(this.state.sub_total+this.state.tax_amount)-discount
                                        } else {
                                            total_amount=(this.state.sub_total+this.state.tax_amount)-parseInt(this.state.discount);
                                            discount=this.state.discount
                                            // total_amount=this.state.sub_total-discount
                                        }
                                       

                                        this.setState({discountType:itemValue,total_amount,discount_amount:discount})
                                    }
                                ...
                                >
                                     <Picker.Item style={{fontSize:13}} color="#929497" label="Percentage" value="Percentage" /> 
                                     <Picker.Item style={{fontSize:13}} color="#929497" label="Fixed" value="Fixed" />
                            ...
                            <View/>
                                ...
                                <TextInput value={this.state.discount} onChangeText={(text)=>this.handleChangeDiscount(text)} keyboardType="numeric" style={{height:52, marginLeft:10,color:"#929497",fontSize:13, backgroundColor:"#fff",borderBottomColor:"#E6E6E6"}} placeholder="Enter discount" />
                        ...
                        <View style={[styles.OrderDetailContainer,{marginTop:12}]}>
                            <View style={[ styles.OrderDetailHeadingRow,{padding:10}]}>
                                <Text style={[{}, styles.OrderDetailHeadingRowText]}>Tax(%)</Text>
                            ...
                            <View style={{ borderWidth: 0.25, borderColor: '#E6E6E6', width: width - 20, alignSelf: 'center', marginTop: 10 }}></View>
                                ...
                                <TextInput value={this.state.tax_percent}   onChangeText={(text)=>this.handleChangeTax(text)} keyboardType="numeric" style={{alignSelf:"flex-start",width:width-70, height:52, marginLeft:10,color:"#929497",fontSize:13, backgroundColor:"#fff",borderBottomColor:"#E6E6E6"}} placeholder="Enter Tax " />
                        ...
                        <View style={{ backgroundColor: '#fff', width: width - 20, alignSelf: 'center', marginTop: 10, borderRadius: 10, paddingBottom: 10 }}>
                            <View style={[{ borderBottomWidth: 0.25 }, styles.subTotleRowView]}>
                                ...
                                <View style={[{}, styles.subTotleColumn1View]}>
                                    <Text style={[ styles.subTotleColumn1Text,{marginBottom:12}]}>Subtotal:</Text>
                                 
                                    <Text style={[{}, styles.subTotleColumn1Text,{marginBottom:12}]}>VAT</Text>
                                        <Text style={[{}, styles.subTotleColumn1Text,{marginBottom:12}]}>Discount</Text>
                                ...
                                <View style={[{}, styles.subTotleColumn2View]}>
                                    <Text style={[{}, styles.subTotleColumn2Text,{marginBottom:12}]}>{this.props.currency.currency + " " + this.state.sub_total}</Text>
                                  
                                        <Text style={[{}, styles.subTotleColumn2Text,{marginBottom:12}]}>{this.props.currency.currency } {this.state.tax_amount}</Text>
                                        
                                        <Text style={[{}, styles.subTotleColumn2Text,{marginBottom:12}]}>{this.props.currency.currency } {this.state.discount_amount}</Text>
                                        <Text style={[{}, styles.subTotleColumn2Text,{marginBottom:12}]}>{this.props.currency.currency} {this.state.total_amount}</Text>
                                </View>
                            </View>
                            <View style={{ flexDirection: 'row', width: width - 50, alignSelf: 'center', marginVertical: 10 }}>
                                ...
                                >
                                    ...
                                    <Icon name="edit" color="#B1272C" size={20}/>
                                        ...
                                        <Text style={{ color: '#929497', fontSize: 13, marginLeft: 5, fontWeight: 'bold' }}>Add a note</Text>
                                    </View>}
                ...
                <View style={{backgroundColor:"#fff",height:70,width:"100%",flexDirection:"row",alignItems:"center",justifyContent:"center"}}>
                <TouchableOpacity
                    onPress={() => this.sendQuickInvoice(0)}
                    style={[ styles.btnContinuueView,{width:width-243,marginRight:20,backgroundColor:"#fff",borderColor:"#B1272C",borderWidth:1}]}>
                ...
                >
                    ...
                    <ScrollView
                        style={{height:height-350, alignSelf: 'center', backgroundColor: '#fff', width: width - 50,  borderRadius: 10, flexDirection: 'column' ,padding:25}}
                    >
                        <View style={{ flexDirection: 'row', marginBottom: 30 }}>
                            <Text style={{ color: '#2F2E7C', fontWeight: 'bold', fontSize: 15 }}>ADD PRODUCT OR SERVICE</Text>
                        </View>
                        <View>
                        <TextInput value={this.state.product_name} onChangeText={(text)=>this.setState({product_name:text})} placeholder="Enter Product or Service" style={{ paddingLeft:0, height:30,backgroundColor:"#fff",fontSize:13}} />
                        <View style={{flexDirection:"row",justifyContent:"space-between",marginTop:20}}>
                        <TextInput keyboardType="numeric" value={this.state.quantity} onChangeText={(text)=>{
                            
                            this.setState({quantity:text,modal_total:parseInt(text) *this.state.price})
                            }} placeholder="Quantity" style={{paddingLeft:0, height:30,backgroundColor:"#fff",fontSize:13}} />
                        <TextInput keyboardType="numeric"  value={this.state.price} onChangeText={(text)=>this.setState({price:text,modal_total:parseInt(text) *this.state.quantity})} placeholder="Price(0.00)" style={{paddingLeft:0, height:30,backgroundColor:"#fff",fontSize:13}} />
                    ...
                    <View style={{flexDirection:"row",justifyContent:"space-between",marginTop:23}}>
                        ...
                        <Text style={{fontWeight:"bold"}}>{this.props.currency.currency}{this.state.modal_total.toString()}.00</Text>
                    </View>
                        ...
                        <View
                            ...
                            >
                                <TouchableOpacity
                                    style={{justifyContent:"center",alignItems:"center", backgroundColor: '#B1272C', paddingVertical: 15, padding: 40, borderRadius: 100 }}
                                    onPress={() => this.state.isEdit?this.editProductToArray(): this.pushProductToArray()}
                                >
                            ...
                            >
                                <TouchableOpacity
                                    style={{ justifyContent:"center",alignItems:"center", backgroundColor: '#fff', paddingVertical: 15, padding: 30, borderRadius: 100, borderWidth: 1, borderColor: '#E6E6E6' }}
                                    onPress={() => { this.setState({ isShowProductModal: false }) }}
                                >
                                    <Text style={{ color: '#E6E6E6', paddingHorizontal: 10 }}>Cancel</Text>
                                </TouchableOpacity>
                            </View>
