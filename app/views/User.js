                            ...
                            <View style={{ flexDirection: 'row', width: width,marginTop:10 }}>
                                ...
                                <View style={{flex:1,justifyContent:'center',alignItems:'center'}}>
                                    <Image
                                        ...
                                        source={this.state.avatar=='' ||this.state.avatar==null?require('../images/profilepic.png'):{uri:this.state.avatar}}
    ...
    <View>
        <View style={{ flexDirection: 'row', width: width,marginTop:10 }}>
            <View style={{ flex: 1 }}>
                ...
                <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{this.state.profile.companyName}</Text>
                <Text style={[{}, styles.userInfoLable]}>Company Email</Text>
                <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{this.state.profile.email}</Text>
            </View>
                            ...
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{this.state.profile.country}</Text>
                            </View>

                            <View>
                            <Text style={[{}, styles.userInfoLable]}>Business Sector</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{this.state.profile.sectortype}</Text>
                            </View>
                            </View>

                            <View style={{borderColor:"#D8D8D8",borderWidth:1,marginBottom:10,marginTop:20}}/>
                            </View>

                            <Text style={[ styles.userInfoLable,{color:"#2F2E7C",marginBottom:10}]}>Business Locations</Text>
                            {
                                this.state.profile.merchantBusinessStructure && this.state.profile.merchantBusinessStructure.length>0 &&this.state.profile.merchantBusinessStructure.map(business=>{
                                    return(
                                        <View style={{flexDirection:"row",justifyContent:"space-between",marginBottom:10}}>
                            <View>
                            <Text style={[{}, styles.userInfoLable]}>City</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{business.city}</Text>
                            </View>

                            <View>
                            <Text style={[{}, styles.userInfoLable]}>Location</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{business.location}</Text>
                            </View>
                            </View>
                                    )
                            ...
                            <View style={{borderColor:"#D8D8D8",borderWidth:1,marginBottom:10,marginTop:10}}/>
                            <Text style={[ styles.userInfoLable,{color:"#2F2E7C",marginBottom:10}]}>Contact Persons</Text>
                            { this.state.profile.contact && this.state.profile.contact>0 && this.state.profile.contact.map(contact=>{
                                return(
                                    <View style={{marginBottom:10}}>
                                         <Text style={[{}, styles.userInfoLable]}>Fullname</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{contact.name}</Text>
                            <Text style={[{}, styles.userInfoLable]}>Email</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{contact.email}</Text>
                            <Text style={[{}, styles.userInfoLable]}>Phone number</Text>
                            <Text style={[{ color: '#4E4D4D', marginBottom: 10 }, fontStyles.normal15]}>{contact.phone}</Text>
    ...
    viewThree(){
        return (
            <View style={{paddingBottom:40}}>
                <View style={{justifyContent:"space-between", flexDirection: 'row', width: width,marginTop:10 }}>
<View>
    <Text style={{fontWeight:"bold"}}>{this.state.first_name}</Text>
    <Text style={{color:"#929497",fontSize:12}}>{this.state.email}</Text>
</View>
<View style={{justifyContent:"center"}}>
    <Text style={{fontWeight:"bold"}}>USER</Text>
    <View style={{marginRight:50, backgroundColor:"#DAF8EC",justifyContent:"center",alignItems:"center",padding:5,borderRadius:10,width:80}}>
...
<Text style={{fontWeight:"bold",color:"#4E4D4D",fontSize:20}}>To add more users to your account , use the web</Text>

<TouchableOpacity
 style={{borderWidth:1, marginTop:15, backgroundColor: '#fff',borderColor:"#2F2E7C", paddingVertical: 10, padding: 15, borderRadius: 100 }}
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    <View>
                        <View style={[{borderRadius:10}, styles.contentContainer]}>
                            <View style={[styles.tabView,{borderBottomColor:"#E6E6E6",borderBottomWidth:1,}]}>
                            <TouchableOpacity
                            style={{ flex: 1,borderBottomWidth:2, borderBottomColor: this.state.tabViewIndex === 1 ? '#B1272C' : '#E6E6E6', paddingVertical: 0 }}
                            onPress={() => { this.setState({ tabViewIndex: 1 }) }}
                        >
                            <Text style={{ color: this.state.tabViewIndex === 1 ? '#B1272C' : '#929497', fontWeight: 'bold', textAlign: 'center' }}>User Profile</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ flex: 1, borderBottomWidth:2,borderBottomColor: this.state.tabViewIndex === 2 ? '#B1272C' : '#E6E6E6', paddingVertical: 5 }}
                            onPress={() => { this.setState({ tabViewIndex: 2 }) }}
                        >
                            <Text style={{ color: this.state.tabViewIndex === 2 ? '#B1272C' : '#929497', fontWeight: 'bold', textAlign: 'center' }}>Company Profile</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ flex: 1,borderBottomWidth:2, borderBottomColor: this.state.tabViewIndex === 3 ? '#B1272C' : '#E6E6E6', paddingVertical: 5 }}
                            onPress={() => { this.setState({ tabViewIndex: 3 }) }}
                        >
                            <Text style={{ color: this.state.tabViewIndex === 3 ? '#B1272C' : '#929497', textAlign: 'center',fontWeight: 'bold' }}>Users</Text>
                        </TouchableOpacity>
                            </View>
                            <this.tabView />
