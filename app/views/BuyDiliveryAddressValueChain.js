...
import { SET_USER, LOGOUT_USER, SET_DELIVERY_ADDRESS ,ADD_DELIVERY_FEE_TO_COST} from '../redux/constants/index';
...
class BuyDiliveryAddressValueChain extends React.Component {
    ...
    submitAddress() {
        ...
        const url=this.state.lgas_id==0? `${Constants.deliveryCost}?order_amount=${this.props.cart.total_price_with_tax}&country_id=${this.state.country_id}&state_id=${this.state.state_id}`: `${Constants.deliveryCost}?order_amount=${this.props.cart.total_price_with_tax}&country_id=${this.state.country_id}&state_id=${this.state.state_id}&lga_id=${this.state.lgas_id}`
        fetch(url, postData)
            .then(response => response.json())
            .then(async responseJson => {
            ...
            <View style={[{}, styles.mainView]}>
                ...
                <ScrollView>
                    <View>
                                ...
                                <View>
                                    ...
                                    <View style={[{}, styles.formRow]}>
                                        ...
                                        <Picker
                                         ...
                                         onValueChange={(itemValue,itemLabel, itemIndex) => this.onSelectCountry(itemValue,itemIndex)
                                          }
                                        selectedValue={this.state.country_id} >
                                            <Picker.Item label="Select Country" value="" />
                                            {this.state.countries_arr.map(elem=>{
                                                console.log("df#",elem)
                                              return  <Picker.Item label={elem.label} value={elem.value} />
                                            })}
                                        </Picker>
                        ...
                        >
                            <Text style={{ color: '#fff' }}>{this.state.countries_arr.length==0 && this.state.hasFetch?'Go Back':'Submit'}</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
            </View>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        addDeliveryToTotal:(value)=> dispatch({ type: ADD_DELIVERY_FEE_TO_COST, value: value })
    }
};
export default connect(mapStateToProps, mapDispatchToProps)(BuyDiliveryAddressValueChain)
