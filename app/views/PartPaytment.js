...
class PartPaytment extends React.Component {
  ...
  required_fields_check() {
    ...
    if (
      this.state.part_amount_request < 1 ||
      this.state.part_amount_request == ''
    ) {
      Alert.alert('Invalid payable amount');
      return false;
    } else if (
      this.state.part_payment_balance_due_date == 0 ||
      this.state.part_payment_balance_due_date == ''
    ) {
  ...
  resendInvoice(bodyOrder) {
   ...
   // let url = Constants.orderslist + '/' + this.state.order.id + '?action=send_invoice';
    fetch(Constants.orderslist, postData)
      ...
      .then(async responseJson => {
        console.log(
          'create_order_id responseJson responseJson!!!!!!!!!!!',
          responseJson,
        );
  ...
  get_payable_amount(amount_to_pay_now) {
    ...
    if (amount_to_pay_now == '') {
      ...
      if (amount_to_pay_now.charAt(0) + '' == '0') {
        amount_to_pay_now = amount_to_pay_now.substring(
          1,
          amount_to_pay_now.length,
        );
    ...
    if (
      amount_to_pay_now.split('.').length > 2 ||
    ...
    if (
      parseFloat(amount_to_pay_now) >= total_amount &&
      this.state.payment_option_selected == 'fixed_amount'
    ) {
        ...
        <ScrollView>
          ...
          <TouchableOpacity onPress={() => _that.props.navigation.goBack()}>
            ...
            <View style={[{paddingHorizontal: 10}, styles.balanceHeadingView]}>
              <Text
                style={[
                  {color: '#929497', alignSelf: 'center'},
                  fontStyles.normal15,
                ]}>
                TOTAL AMOUNT
              </Text>
              <View style={[{backgroundColor: '#DAF8EC'}, styles.balanceView]}>
                <Text style={[{color: '#4E4D4D'}, fontStyles.bold25]}>
                  {_that.props.currency.currency +
                    ' ' +
                    parseFloat(params.amount_payable + '').toFixed(2)}
                </Text>
              </View>
              <Text style={[{color: '#929497', fontSize: 8}]}>
                Type of payment
              </Text>
              <DropDownPicker
              ...
              <TextInput
                label={
                  _that.state.payment_option_selected == 'fixed_amount'
                    ? 'Amount of pay'
                    : 'Percentage'
                }
                ...
                onChangeText={amount_to_pay_now =>
                  _that.get_payable_amount(amount_to_pay_now)
                }
                value={
                  _that.state.part_amount_request == '0'
                    ? ''
                    : _that.state.part_amount_request
                }
              />
              <Text
                style={[
                  {color: '#929497', alignSelf: 'center', marginTop: 20},
                  fontStyles.normal15,
                ]}>
                Amount to pay now
              </Text>
              <View style={[{backgroundColor: '#FFF4F4'}, styles.balanceView]}>
                <Text style={[{color: '#4E4D4D'}, fontStyles.bold25]}>
                  {_that.props.currency.currency +
                    ' ' +
                    parseFloat(_that.state.amount_to_pay_now).toFixed(2)}
                </Text>
              </View>
              <View
                style={[
                ...
                <Text style={[{color: '#B1272C'}, fontStyles.normal15]}>
                  Balance amount of{' '}
                </Text>
                <Text style={[{color: '#B1272C'}, fontStyles.bold15]}>
                  {_that.props.currency.currency +
                    ' ' +
                    parseFloat(_that.state.balance_amount).toFixed(2)}{' '}
                </Text>
                <Text style={[{color: '#B1272C'}, fontStyles.normal15]}>
                  is due
                </Text>
              ...
              <TouchableOpacity
                ...
                />
                <Text style={[{color: '#909090'}, fontStyles.normal12]}>
                  {_that.state.selected_date}
                </Text>
                <Icon
              ...
              <View style={[{}, styles.btnRow]}>
                <View style={[{}, styles.btnColumn]}>
                  <TouchableOpacity
                    ...
                    style={[
                      {borderColor: '#B1272C', borderWidth: 1},
                      styles.btnTouch,
                    ]}>
                    <Text style={[{color: '#B1272C'}, fontStyles.normal15]}>
                      Send Invoice
                ...
                <View style={[{}, styles.btnColumn]}>
                  <TouchableOpacity
                    ...
                    <Text style={[{color: '#fff'}, fontStyles.normal15]}>
                      Pay now
                    </Text>
                  </TouchableOpacity>
                </View>
